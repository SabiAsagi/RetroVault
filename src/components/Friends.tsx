          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {friends.map(f => (
              <div key={f.id} className="bg-vault-surface border border-vault-border rounded-xl p-4 flex items-center justify-between hover:border-vault-border-light transition-colors">
                <Link href={`/profile/${f.nickname || f.name || f.id}`} className="flex items-center gap-3 hover:opacity-80">
                  ...
                  <span className="font-bold text-text-primary">{f.nickname || f.name || '알 수 없는 유저'}</span>
                </Link>
                <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            {requests.map(r => (
              <div key={r.id} className="bg-vault-surface border border-vault-border rounded-xl p-4 flex items-center justify-between">
                <Link href={`/profile/${r.user.nickname || r.user.name || r.user.id}`} className="flex items-center gap-3 hover:opacity-80">
                  ...
                  <span className="font-bold text-text-primary">{r.user.nickname || r.user.name || '알 수 없는 유저'}</span>
                </Link>
                <div className="flex items-center gap-2">
                  <button onClick={async () => {
              ...
              return (
                <div key={msg.id} onClick={() => markAsRead(msg.id)} className={`bg-vault-surface border rounded-xl p-4 transition-all cursor-pointer ${isUnread ? 'border-neon-blue shadow-[0_0_10px_rgba(74,168,255,0.1)]' : 'border-vault-border hover:border-vault-border-light'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <Link href={`/profile/${targetUser.nickname || (targetUser as any).name || targetUser.id}`} className="flex items-center gap-2 hover:text-neon-blue transition-colors">
                      ...
                      <span className="font-bold text-text-primary text-sm">{targetUser.nickname || (targetUser as any).name || '알 수 없는 유저'}</span>
                    </Link>
                    <div className="flex items-center gap-2">
